...
public class BloqueTx implements Comparable<BloqueTx> {
    ...
    public BloqueTx(Transaccion[] transacciones){
        ...
        //como es un nuevo bloque, inicializo todas las variables de la estadistica
        inicializarEstadisticas();

        for(int i = 0; i < transacciones.length; i++){ // O(n)
            handlesUltBloque.add(ultBloque.agregarAtras(transacciones[i]));    // O(1)
            modificarEstadisticas(transacciones[i], true);  //O(1)
        }
        calcularMontoMedio(); //O(1)
        heapUltBloque.Heapify(handlesUltBloque);    //heapificar con algoritmo floyd -> O(n)
    }

    //O(1)
    private void calcularMontoMedio(){
        if(cantTxSinCreacion != 0)
            montoMedio = sumatoriaMontos/cantTxSinCreacion;  //O(1)
        else
            montoMedio = 0;         //O(1)
    }
    ...
    //O(1)
    private void modificarEstadisticas(Transaccion tx, boolean sumar){
        if(tx.id_comprador() != 0){
            if(sumar == true){
                sumatoriaMontos     += tx.monto();  //O(1)
    ...
    public Transaccion hackearTx(){
        ...
        modificarEstadisticas(txEliminar, false); // O(1)
        calcularMontoMedio(); // O(1)
